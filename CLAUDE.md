...
- **NFC dual-record layout: plain "openable" record on top, TagDrop
  manifest record on bottom** (assessed: *plausible, not started*) — see
  [#43](https://github.com/mofosyne/tagdrop/issues/43) for the full
  writeup (proposed approach, tradeoffs, scope). Related to the older
  [#9](https://github.com/mofosyne/tagdrop/issues/9) (generic NFC
  MIME-type support).
- **QR plain-text mode with hash-matched "orphan" raw scans** (assessed:
  *plausible, not started*) — see
  [#44](https://github.com/mofosyne/tagdrop/issues/44) for the full
  writeup (proposed approach, tradeoffs, scope).
